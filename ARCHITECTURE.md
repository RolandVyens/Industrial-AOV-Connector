...
```mermaid
graph TD
    User([User / 用户]) -->|Click 'Cook Nodetree'| Ops[operators/tree_ops.py]
    
    subgraph Data Flow
        Ops --> TreeBuilder[core/node_builder.py : TreeBuilder]
        TreeBuilder --> Analysis[sort_passes.py]
        Analysis --> Dict[ViewLayer Dict]
        TreeBuilder --> Constants[constants.py]
        
        Dict --> TreeBuilder
        TreeBuilder --> Create[Create Nodes]
        TreeBuilder --> Connect[Connect Nodes]
        TreeBuilder --> Arrange[Arrange Nodes]
    end

    subgraph Config
        TreeBuilder --> Prefs[core/preferences.py]
        TreeBuilder --> Props[core/properties.py]
    end
    
    Arrange --> Finish([Ready to Render])
```
