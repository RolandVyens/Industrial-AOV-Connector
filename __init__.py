...
class IDS_AddonPrefs(AddonPreferences):
    ...
    Custom_Prefix: StringProperty(
        name="Custom File Suffix",
        description='You can add custom text and prefix to the file name generated by Industrial AOV Connector, for example use # to customize frame number digits. You can also use $scene$, $file$, $camera$, $version$ as prefix that can be automatically changed to actual names when rendering ($version$ expects there is a "v001" style version number at the last of your blend file name). A good example is "$scene$_$version$_###"',
        default="",
        maxlen=100,
    )  # type: ignore
