language_dict = {
    "zh_CN": {
        ...
        ("*", "Custom File Name Suffix"): "自定义文件名后缀",
        (
            "*",
        ...
        ): "在生成的文件名后面添加自定义字符串，例如使用#来自定义帧号位数。你也可以使用$scene$, $file$, $camera$, $version$等预设标识，渲染输出时会被自动替换成真实名字（$version$需要你的blend文件名最后有“v001”样式的版本号）。一个好例子：$scene$_$version$_###",
        ...
        ): "节点排列时节点间隔的缩放比例，修复节点间隔过宽和空间消耗问题。这是对系统范围的UI缩放的补偿，例如我的Windows使用1.5倍的缩放比例，设置为0.67将生成正确的节点树",
    },
    "zh_HANS": {
        (
            "*",
